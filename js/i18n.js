...
const I18n = (() => {
  ...
  const TRADUCOES = {
    pt: {
      ...
      // -- Tela de login do servidor --
      // -- Leitura qualitativa da correlacao (chaves vindas da API) --
      'corr.nula':'correlação praticamente nula',
      'corr.fraca_positiva':'correlação fraca e positiva','corr.fraca_negativa':'correlação fraca e negativa',
      'corr.moderada_positiva':'correlação moderada e positiva','corr.moderada_negativa':'correlação moderada e negativa',
      'corr.forte_positiva':'correlação forte e positiva','corr.forte_negativa':'correlação forte e negativa',
      'corr.muito_forte_positiva':'correlação muito forte e positiva','corr.muito_forte_negativa':'correlação muito forte e negativa',
      'corr.insuficiente':'amostra insuficiente para correlacionar','corr.sem_variacao':'série sem variação',
      // -- Video do pitch, exportacao e anexos --
      'video.titulo':'Conheça o Protege+ em 5 minutos',
      'video.sub':'Apresentação do projeto e demonstração do sistema em funcionamento: registro de denúncia, painel do servidor, fila de priorização e trilha de auditoria.',
      'video.assistir':'▶ Assistir no YouTube',
      'backlog.titulo':'Backlog de Denúncias',
      'backlog.exportar':'⬇ Exportar CSV',
      'backlog.exportar_nota':'A exportação é gerada pelo servidor, restrita aos perfis Gestor e Administrador, e fica registrada na trilha de auditoria. O arquivo não inclui o relato das denúncias: as perguntas de gestão não precisam do texto, e uma planilha com centenas de relatos circulando por e-mail seria o vazamento que o resto do sistema evita.',
      'evid.titulo':'Anexos da denúncia',
      'evid.carregando':'Carregando…',
      'evid.vazio':'Nenhum arquivo anexado a esta denúncia.',
      'evid.baixar':'⬇ Baixar','evid.remover':'🗑 Remover',
      'evid.baixado':'⬇ Download iniciado. A leitura foi registrada na trilha de auditoria.',
      'evid.motivo':'Motivo da remoção (obrigatório):',
      'evid.removido':'🗑 Anexo removido.',
      // -- Ficha do caso no painel --
      'det.carregando':'Carregando o caso…',
      'det.local':'A ficha completa é servida pela API. No modo local só a situação e a linha do tempo estão disponíveis.',
      'det.nao_encontrado':'Caso não encontrado.',
      'det.relato':'Relato','det.sem_relato':'sem relato registrado',
      'det.endereco':'Endereço','det.sem_endereco':'não informado',
      'det.mascara_dica':'O servidor não enviou o endereço para o seu perfil',
      'det.score':'score','det.urgencia':'urgência',
      'det.anonima':'anônima','det.identificada':'identificada',
      'det.registrada':'Registrada em','det.concluida':'Concluída em',
      'det.resumo_ia':'Leitura do VigIA','det.origem':'origem',
      'det.origem_nota':'A origem da análise é persistida: decisão automatizada precisa ter fundamento declarado e rastreável.',
      'det.anexos':'Anexos','det.sem_historico':'sem eventos registrados',
      'det.editar':'Editar','det.editando':'Editando o caso','det.salvar':'Salvar alterações',
      'det.editar_nota':'A alteração é gravada pelo servidor e registrada na trilha como ALTERACAO_DADOS.',
      'det.salvo':'✅ Caso atualizado. A alteração está na trilha de auditoria.',
      'vigia.ph':'Pergunte ao VigIA — ex.: onde devo concentrar a equipe esta semana?',
      'vigia.perguntar':'Perguntar','vigia.pensando':'Analisando…',
      'vigia.nota':'O contexto enviado à IA contém apenas números agregados — tipo, status, urgência e prazo. Nenhum relato de denúncia sai do sistema.',
      'det.reanalisar':'Reanalisar',
      'det.reanalisado':'🤖 Caso reanalisado. A reanálise ficou registrada na trilha.',
      'det.excluir':'Excluir',
      'det.motivo':'Motivo da exclusão (obrigatório).\n\nO relato e o endereço serão destruídos. O registro permanece, com motivo e data, para preservar a rastreabilidade.',
      'det.excluido':'🗑 Caso excluído e anonimizado. A operação está na trilha de auditoria.',
      'det.auditoria':'Esta leitura foi registrada na trilha de auditoria com o seu usuário, perfil e horário.',
      'login.titulo':'Acesso ao Servidor',
      'login.sub':'Área restrita. A autenticação é feita pela API: a senha é conferida contra o hash BCrypt no servidor, que devolve um token assinado com o seu perfil.',
      'login.email':'E-mail institucional','login.email_ph':'nome@protege.gov.br',
      'login.senha':'Senha','login.senha_ph':'Digite sua senha',
      'login.demo':'💡 Contas de demonstração — cada uma recebe um perfil diferente do servidor:',
      'login.entrar':'Entrar','login.entrando':'Entrando…',
      'login.erro.vazio':'Informe o e-mail e a senha.',
      'login.erro.credencial':'E-mail ou senha incorretos.',
      'login.offline.tit':'Sem servidor? Veja assim mesmo',
      'login.offline.sub':'O painel completo roda no navegador, com as mesmas 125 denúncias da base de demonstração. Relatório estatístico e trilha de auditoria exigem a API no ar, porque são calculados no servidor.',
      'login.offline.btn':'Entrar em modo demonstração',
      'login.erro.demo_api_no_ar':'A API está no ar. Use uma das contas reais para ver o dado do banco.',
      'login.erro.offline':'A API está fora do ar. Sem servidor não há autenticação — suba o backend para acessar o painel.',
      ...
      'est.urg.1':'O tempo cresce à medida que a urgência cai — de','est.urg.2':'na urgência mais alta a',
    ...
    en: {
      ...
      // -- Staff login screen --
      // -- Qualitative reading of the correlation (keys sent by the API) --
      'corr.nula':'practically no correlation',
      'corr.fraca_positiva':'weak positive correlation','corr.fraca_negativa':'weak negative correlation',
      'corr.moderada_positiva':'moderate positive correlation','corr.moderada_negativa':'moderate negative correlation',
      'corr.forte_positiva':'strong positive correlation','corr.forte_negativa':'strong negative correlation',
      'corr.muito_forte_positiva':'very strong positive correlation','corr.muito_forte_negativa':'very strong negative correlation',
      'corr.insuficiente':'sample too small to correlate','corr.sem_variacao':'series with no variation',
      // -- Pitch video, export and attachments --
      'video.titulo':'Get to know Protege+ in 5 minutes',
      'video.sub':'Project pitch and a walkthrough of the running system: filing a report, the staff dashboard, the priority queue and the audit trail.',
      'video.assistir':'▶ Watch on YouTube',
      'backlog.titulo':'Reports Backlog',
      'backlog.exportar':'⬇ Export CSV',
      'backlog.exportar_nota':'The export is generated by the server, restricted to the Manager and Administrator profiles, and recorded in the audit trail. The file does not include the report narratives: management questions do not need the text, and a spreadsheet with hundreds of narratives travelling by email would be the very leak the rest of the system prevents.',
      'evid.titulo':'Report attachments',
      'evid.carregando':'Loading…',
      'evid.vazio':'No file attached to this report.',
      'evid.baixar':'⬇ Download','evid.remover':'🗑 Remove',
      'evid.baixado':'⬇ Download started. The read was recorded in the audit trail.',
      'evid.motivo':'Reason for removal (required):',
      'evid.removido':'🗑 Attachment removed.',
      // -- Case file in the dashboard --
      'det.carregando':'Loading the case…',
      'det.local':'The full case file is served by the API. In local mode only the status and the timeline are available.',
      'det.nao_encontrado':'Case not found.',
      'det.relato':'Report','det.sem_relato':'no report recorded',
      'det.endereco':'Address','det.sem_endereco':'not provided',
      'det.mascara_dica':'The server did not send the address to your profile',
      'det.score':'score','det.urgencia':'urgency',
      'det.anonima':'anonymous','det.identificada':'identified',
      'det.registrada':'Filed on','det.concluida':'Concluded on',
      'det.resumo_ia':'VigIA reading','det.origem':'source',
      'det.origem_nota':'The source of the analysis is persisted: an automated decision must have a declared and traceable basis.',
      'det.anexos':'Attachments','det.sem_historico':'no events recorded',
      'det.editar':'Edit','det.editando':'Editing the case','det.salvar':'Save changes',
      'det.editar_nota':'The change is written by the server and recorded in the trail as ALTERACAO_DADOS.',
      'det.salvo':'✅ Case updated. The change is in the audit trail.',
      'vigia.ph':'Ask VigIA — e.g. where should I concentrate the team this week?',
      'vigia.perguntar':'Ask','vigia.pensando':'Analysing…',
      'vigia.nota':'The context sent to the AI contains only aggregate numbers — type, status, urgency and turnaround. No report narrative leaves the system.',
      'det.reanalisar':'Re-analyse',
      'det.reanalisado':'🤖 Case re-analysed. The re-analysis was recorded in the trail.',
      'det.excluir':'Delete',
      'det.motivo':'Reason for deletion (required).\n\nThe report and the address will be destroyed. The record remains, with reason and date, to preserve traceability.',
      'det.excluido':'🗑 Case deleted and anonymised. The operation is in the audit trail.',
      'det.auditoria':'This read was recorded in the audit trail with your user, profile and timestamp.',
      'login.titulo':'Staff Sign In',
      'login.sub':'Restricted area. Authentication happens in the API: the password is checked against the BCrypt hash on the server, which returns a token signed with your profile.',
      'login.email':'Institutional email','login.email_ph':'name@protege.gov.br',
      'login.senha':'Password','login.senha_ph':'Enter your password',
      'login.demo':'💡 Demo accounts — each one receives a different profile from the server:',
      'login.entrar':'Sign in','login.entrando':'Signing in…',
      'login.erro.vazio':'Enter your email and password.',
      'login.erro.credencial':'Incorrect email or password.',
      'login.offline.tit':'No server? See it anyway',
      'login.offline.sub':'The full panel runs in the browser, with the same 125 reports from the demo dataset. The statistical report and the audit trail require the API, because they are computed on the server.',
      'login.offline.btn':'Enter demonstration mode',
      'login.erro.demo_api_no_ar':'The API is up. Use one of the real accounts to see database data.',
      'login.erro.offline':'The API is down. Without a server there is no authentication — start the backend to reach the panel.',
      ...
      'est.urg.1':'Time grows as urgency falls — from','est.urg.2':'at the highest urgency to',
