...
function getAllDenuncias() {
  ...
  if (typeof LGPD !== 'undefined' && LGPD.decifrarDenuncia) {
    return todas.map(d => LGPD.decifrarDenuncia(d));
  }
  return todas;
}

// Grava o array `denuncias` no localStorage CIFRANDO os campos sensíveis.
// A memória (`denuncias`) permanece em texto claro; só o armazenamento é cifrado.
function persistirDenuncias() {
  try {
    let payload;
    if (typeof LGPD !== 'undefined' && LGPD.cifrarDenuncia) {
      payload = denuncias.map(d => LGPD.cifrarDenuncia(d));
    } else {
      payload = denuncias;
    }
    localStorage.setItem('denuncias', JSON.stringify(payload));
    return true;
  } catch (e) {
    console.warn('Erro ao persistir denúncias:', e);
    return false;
  }
}
...
function navigate(page) {
  ...
  // Botão de pânico só faz sentido na área pública (landing), não no painel do gestor
  if (typeof Panico !== 'undefined') {
    if (page === 'dashboard') Panico.esconder();
    else Panico.mostrar();
  }
  ...
  if (page === 'dashboard') {
    ...
    renderBacklog();
    renderStatusTable();
    if (typeof VigIA !== 'undefined') VigIA.analisar();
...
function gerarQRCode(protocolo) {
  ...
  label.style.cssText = 'font-size:12px;color:#64748b;margin-bottom:6px;font-weight:500;';

  const img = document.createElement('img');
  img.src   = url;
  img.alt   = 'QR Code do protocolo ' + protocolo;
  img.style.cssText = 'width:120px;height:120px;border:2px solid #e2e8f0;border-radius:10px;padding:5px;background:#fff;';

  const dica = document.createElement('p');
  dica.textContent = 'Salve esta imagem para consultar depois';
  dica.style.cssText = 'font-size:11px;color:#94a3b8;margin-top:6px;';

  const btnSalvar = document.createElement('button');
  btnSalvar.textContent = '💾 Salvar QR Code';
  btnSalvar.style.cssText = 'margin-top:8px;padding:7px 16px;background:#1A4DA0;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600;';
  btnSalvar.onclick = () => {
...
async function enviarDenuncia() {
  ...
  const denuncia = {
    ...
    nivelAnonimato: parseInt(document.getElementById('f-nivel-anonimato')?.value || '2', 10),
  ...
  // Nível 2/3: gera código secreto de acompanhamento (elo de mão dupla)
  if (typeof Anonimato !== 'undefined' &&
      (denuncia.nivelAnonimato === 2 || denuncia.nivelAnonimato === 3)) {
    denuncia.codigoAcompanhamento = Anonimato.gerarCodigo();
  }
  ...
  // Código de acompanhamento (nível 2/3) na tela de sucesso
  const codigoBloco = document.getElementById('modal-codigo-bloco');
  if (codigoBloco) {
    if (denuncia.codigoAcompanhamento) {
      document.getElementById('modal-codigo-valor').textContent = denuncia.codigoAcompanhamento;
      codigoBloco.style.display = 'block';
      window._ultimoCodigo = denuncia.codigoAcompanhamento;
      window._ultimoProtocolo = id;
    } else {
      codigoBloco.style.display = 'none';
    }
  }
...
function showDashTab(tab, el) {
  // Controle de acesso: analista não acessa abas restritas
  if (typeof Acesso !== 'undefined' && !Acesso.podeVerAba(tab)) {
...
function _filtrarDenuncias() {
  const tipo    = document.getElementById('df-tipo')?.value || '';
  const status  = document.getElementById('df-status')?.value || '';
  const periodo = document.getElementById('df-periodo')?.value || '';

  let lista = getAllDenuncias();
  ...
  if (periodo) {
    ...
    lista = lista.filter(d => {
      ...
      const t = new Date(d.data + 'T00:00:00').getTime();
...
function aplicarFiltrosDash() {
  const lista = _filtrarDenuncias();
  updateKPIs(lista);
  initCharts(lista);

  const total = getAllDenuncias().length;
  const res = document.getElementById('df-resultado');
  if (res) {
    res.textContent = (lista.length === total)
      ? `${total} denúncias`
      : `${lista.length} de ${total} denúncias`;
  }
}

function limparFiltrosDash() {
  ['df-tipo', 'df-status', 'df-periodo'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  aplicarFiltrosDash();
}

function initCharts(listaFiltrada) {
  ...
  // ── NOVO: Nível de anonimato escolhido ────────────────────
  const niv = { 1: 0, 2: 0, 3: 0 };
  all.forEach(d => {
    const n = d.nivelAnonimato || (d.anonimo ? 1 : 3);
    if (niv[n] !== undefined) niv[n]++;
  });
  _createChart('chart-anonimato', 'doughnut', {
    labels:   ['Anônimo total', 'Anônimo c/ canal', 'Identificado'],
    datasets: [{ data: [niv[1], niv[2], niv[3]], backgroundColor: ['#64748B','#2563EB','#16A34A'], borderWidth: 0 }]
  });

  // ── NOVO: Prioridade (emergência vs normal) ───────────────
  const emerg  = all.filter(d => d.emergencia).length;
  const normal = all.length - emerg;
  _createChart('chart-prioridade', 'bar', {
    labels:   ['Emergência', 'Normal'],
    datasets: [{ data: [emerg, normal], backgroundColor: ['#DC2626','#2563EB'], borderRadius: 6, borderSkipped: false }]
  });
}
...
function renderStatusTable() {
  ...
  tbody.innerHTML = all.filter(d => d.id && d.status && d.tipo).map(dRaw => {
    const d = (typeof Acesso !== 'undefined') ? Acesso.mascarar(dRaw) : dRaw;
    const key   = d.id.replace(/[^a-zA-Z0-9]/g, '');
    const color = STATUS_COLORS[d.status] || '#64748B';

    // Identidade conforme o papel (master vê o nome, analista vê protegido)
    let identidade = '';
    const _niv = d.nivelAnonimato || (d.anonimo ? 1 : 3);
    if (_niv === 3) {
      identidade = `<br><span class="ident-info">👤 ${d.nome || '—'}</span>`;
    }

    // Badge de nível de anonimato
    let nivelBadge = '';
    let canalBtn = '';
    if (typeof Anonimato !== 'undefined') {
      const r = Anonimato.rotulo(d.nivelAnonimato || (d.anonimo ? 1 : 3));
      nivelBadge = `<br><span class="nivel-badge" style="color:${r.cor};margin-top:4px">${r.icon} ${r.nome}</span>`;

      // Botão de canal seguro (só níveis 2 e 3)
      if (Anonimato.temCanal(d)) {
        const pend = Anonimato.temPendencia(d, 'gestor');
        const ident = d.codigoAcompanhamento || d.id;
        canalBtn = `<button class="btn-elo-gestor" onclick="EloGestor.abrir('${ident}')">
          🔐 Canal${pend ? ' <span class="elo-pendencia">● nova</span>' : ''}
        </button>`;
      }
    }

    return `<tr id="row-${key}">
      <td><strong>${d.id}</strong>${nivelBadge}${identidade}</td>
      <td>${d.tipo}</td>
      ...
      <td>
        <button class="btn btn-primary btn-sm" onclick="StatusConfirm.solicitar('${d.id}','${key}')">💾 Salvar</button>
        ${canalBtn}
      </td>
    </tr>`;
  }).join('');
