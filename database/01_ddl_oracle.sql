...
-- =============================================================================
--  BLOCO 6 - ANEXOS, NOTIFICACAO E CHATBOT
--
--  EVIDENCIA atende as US28 a US30 e esta IMPLEMENTADA nesta fase: upload de
--  anexos pela API, substituindo o armazenamento em Base64 no localStorage da
--  Fase 4, que esbarrava no teto de aproximadamente 5 MB por origem do
--  navegador. As colunas abaixo espelham a entidade JPA Evidencia.
--
--  NOTIFICACAO e CHATBOT_INTERACAO seguem previstas no backlog, sem entidade.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- EVIDENCIA - arquivos anexados a denuncia
--
-- A tabela guarda METADADO. O binario fica no sistema de arquivos, fora da
-- pasta servida pelo servidor web, com nome gerado. Gravar 5 MB por linha
-- incharia o backup, atrasaria toda consulta a tabela e transformaria o dump
-- do banco num vazamento de todas as evidencias de uma vez.
--
-- nome_original e o nome que o usuario enviou, ja higienizado, e existe
-- apenas para ser exibido na tela. nome_armazenado e o nome real no disco,
-- um UUID sem extensao. Manter os dois separados e o que torna a travessia
-- de diretorio impossivel: o texto sob controle do usuario nunca chega ao
-- sistema de arquivos.
-- -----------------------------------------------------------------------------
CREATE TABLE EVIDENCIA (
    ...
    nome_original    VARCHAR2(160)  NOT NULL,
    nome_armazenado  VARCHAR2(64)   NOT NULL,

    -- Tipo confirmado pela ASSINATURA do arquivo, nao pela extensao nem pelo
    -- cabecalho Content-Type - ambos sao escolhidos por quem envia.
    tipo_conteudo    VARCHAR2(60)   NOT NULL,
    tamanho_bytes    NUMBER(12)     NOT NULL,

    -- SHA-256 do conteudo recebido. Detecta troca do arquivo no disco e
    -- identifica reenvio do mesmo arquivo sem compara-lo byte a byte.
    hash_sha256      CHAR(64)       NOT NULL,

    enviado_em       TIMESTAMP(3)   DEFAULT SYSTIMESTAMP NOT NULL,

    removida         NUMBER(1)      DEFAULT 0 NOT NULL,
    removida_em      TIMESTAMP(3),
    motivo_remocao   VARCHAR2(300),

    CONSTRAINT evidencia_pk            PRIMARY KEY (id),
    CONSTRAINT evidencia_armazenado_uk UNIQUE (nome_armazenado),
    CONSTRAINT evidencia_denuncia_fk   FOREIGN KEY (denuncia_id)
        REFERENCES DENUNCIA (id) ON DELETE CASCADE,

    -- Lista branca tambem no banco: se algum dia a validacao da aplicacao for
    -- contornada, nao entra registro de formato que o canal nao aceita.
    CONSTRAINT ck_evidencia_tipo CHECK (tipo_conteudo IN (
        'image/jpeg','image/png','image/webp','application/pdf',
        'audio/mpeg','audio/ogg','audio/webm','video/mp4')),
    ...
    CONSTRAINT ck_evidencia_hash     CHECK (REGEXP_LIKE (hash_sha256, '^[0-9a-f]{64}$')),
    CONSTRAINT ck_evidencia_removida CHECK (removida IN (0,1)),

    -- Remocao exige motivo e data, como a exclusao de denuncia.
    CONSTRAINT ck_evidencia_remocao CHECK (
        removida = 0 OR (motivo_remocao IS NOT NULL AND removida_em IS NOT NULL))
);

COMMENT ON TABLE  EVIDENCIA                 IS 'Metadado dos arquivos anexados. O binario fica no disco, fora da pasta publica';
COMMENT ON COLUMN EVIDENCIA.nome_original   IS 'Nome enviado pelo usuario, ja higienizado. Serve apenas para exibicao';
COMMENT ON COLUMN EVIDENCIA.nome_armazenado IS 'UUID sem extensao. E o nome real no disco - o nome do usuario nunca chega la';
COMMENT ON COLUMN EVIDENCIA.tipo_conteudo   IS 'Detectado pela assinatura do conteudo, nao pela extensao';
COMMENT ON COLUMN EVIDENCIA.hash_sha256     IS 'Conferido na leitura: arquivo trocado no disco tem o download recusado';
COMMENT ON COLUMN EVIDENCIA.removida        IS 'Remocao logica. O binario e apagado, o registro fica para rastreabilidade';
...
--  primeira). CHECK: 5 -> 30. Indices: 8 -> 13. Toda coluna numerica passou a
--  declarar precisao, e a coluna DENUNCIA.tipo deixou de ser texto solto para
--  se tornar chave estrangeira do dominio.
--
--  Nesta fase EVIDENCIA deixou de ser tabela apenas prevista e passou a ter
--  entidade JPA, endpoints de upload e download e restricoes proprias - entre
--  elas a lista branca de formatos, repetida no banco para o caso de a
--  validacao da aplicacao ser algum dia contornada.
-- =============================================================================
